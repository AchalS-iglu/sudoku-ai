...
#calculating possible values for all the empty spaces
...
#calculating total number of possible solutions
combinations = 1
for i in range(81):
    combinations = combinations*len(puzzle_values[i])
print("{:.2e}".format(combinations))

#using brute force to go through all the possible combinations and checking them, only calculates the amount of time it would take
...
#bruteForce_check(puzzle_values) #unfeasble amount of time required
...
#brute force but goes cell by cell
...
solve(sudoku_df.iloc[0,0],puzzle_values) #much faster almost instant